...
class Harvest(BaseModel):
    ...
    yield_amount: Decimal = Field(
        ..., 
        gt=0, 
        description="Amount harvested (must be positive)"
    )
...
class Plant(BaseModel):
    ...
    harvests: List[Harvest] = Field(default_factory=list, description="List of harvests")
    total_yield: Decimal = Field(
        default=Decimal('0'), 
        description="Total yield across all harvests"
    )
...
class GardenDB(BaseModel):
    ...
    plants: Dict[str, Plant] = Field(default_factory=dict, description="Plants in the garden")
    ...
    def add_harvest(self, plant_name: str, yield_amount: Decimal, notes: Optional[str] = None) -> None:
