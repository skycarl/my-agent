...
class TelegramBot:
    ...
    def __init__(self):
        ...
        logger.info(f"TelegramBot initialized successfully with authorized user ID: {self.authorized_user_id}")
    ...
    def _log_unauthorized_access(self, update: Update, action: str = "message") -> None:
        ...
        if not update.message or not update.message.from_user:
            logger.warning(f"Unauthorized {action} attempt with incomplete user information")
            return
    ...
    def _truncate_conversation_history(self, user_id: int) -> None:
        ...
        if user_id in self.conversation_history:
            ...
            if len(history) > self.max_conversation_history:
                ...
                self.conversation_history[user_id] = history[-self.max_conversation_history:]
                logger.debug(f"Truncated conversation history for user {user_id} to {self.max_conversation_history} messages")
...
Just send me any message and I'll respond using AI!
    ...
    async def clear_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        username = update.message.from_user.username
        logger.info(f"Authorized user {user_id} ({username}) requested conversation clear")
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            logger.debug(f"Processing message from authorized user {message_data.user_id} ({message_data.username}): {message_data.text}")
            
            # Add user message to conversation history
            self._add_message_to_history(message_data.user_id, "user", message_data.text)
            
            # Get conversation history
            conversation_history = self._get_conversation_history(message_data.user_id)
            
            # Send typing action
            await context.bot.send_chat_action(chat_id=message_data.chat_id, action="typing")
            
            # Call the API with conversation history
            response_text = await self.get_ai_response(conversation_history)
            
            # Add assistant response to conversation history
            self._add_message_to_history(message_data.user_id, "assistant", response_text)
            
            # Send response back to user
            await update.message.reply_text(response_text)
            
            logger.debug(f"Response sent to authorized user {message_data.user_id}: {response_text}")
    ...
    async def get_ai_response(self, conversation_history: List[APIMessage]) -> str:
        ...
        try:
            ...
            logger.debug(f"Making API call to {self.app_url}/responses with {len(conversation_history)} messages")
            logger.debug(f"Using X-Token: '{self.x_token}' (length: {len(self.x_token)})")
